...
const CHUNKS_DIR = join(TEMP_DIR, "node_modules/@fluentui/react-icons/lib/sizedIcons");
 ...
 */
function parseBracketed(source: string, start: number): { value: string; end: number } {
  const stack: number[] = [];
  let inString: string | null = null;
  let i = start;
